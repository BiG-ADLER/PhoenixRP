...
MySQL.ready(function()
	local result = MySQL.query.await('SELECT * FROM gang_account')

	for i=1, #result, 1 do
		local name   = result[i].name
		local shared = result[i].shared

		local result2 = MySQL.query.await('SELECT * FROM gang_account_data WHERE gang_name = @gang_name', {
			['@gang_name'] = name
		})
		...
		if shared ~= 0 then
			...
			if #result2 == 0 then
				MySQL.update.await('INSERT INTO gang_account_data (gang_name, money, owner) VALUES (@gang_name, @money, NULL)',
				{
					['@gang_name'] 		= name,
					['@money']        	= 0
				})
...
AddEventHandler('gangaccount:updateGangAccount', function(name, cb)
	...
	local result = MySQL.query.await('SELECT * FROM gang_account_data WHERE gang_name = @gang_name', {
		['@gang_name'] = name
	})
