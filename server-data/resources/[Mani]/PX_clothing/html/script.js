...
ClothingMenu.ResetItemTexture = function(obj, category) {
    ...
    $.post('http://PX_clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        $.post('http://PX_clothing/Sound');
        if (hasTracker && clothingCategory == "accessory") {
            $.post('http://PX_clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('http://PX_clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('http://PX_clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('http://PX_clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('http://PX_clothing/rotateRight');
            break;
        case 65: // A
            $.post('http://PX_clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    ClothingMenu.Close();
    $.post('http://PX_clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    ClothingMenu.Close();
    $.post('http://PX_clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('http://PX_clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('http://PX_clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('http://PX_clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('http://PX_clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});