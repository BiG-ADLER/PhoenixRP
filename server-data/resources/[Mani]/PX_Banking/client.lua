            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						-- exports['okokNotify']:Alert("BANK", "", 5000, 'info')
						ESX.ShowNotification("Shoma Aval Bayad Baraye Cart Khod Ramz Bezarid [ Be Bank Morajee Konid ] ", 'error')
					end
				end)
            end
	...
	elseif data.action == "transfer" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				ESX.ShowNotification("This IBAN does not exist", 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				ESX.ShowNotification("This IBAN is already in use", 'error')
		...
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				ESX.ShowNotification("You can only use numbers on your IBAN")
	...
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				ESX.ShowNotification("Your PIN needs to be 4 digits long", 'error')

			end
		else
			-- exports['okokNotify']:Alert("BANK", "You can only use numbers", 5000, 'info')
			ESX.ShowNotification("You can only use numbers", 'error')
