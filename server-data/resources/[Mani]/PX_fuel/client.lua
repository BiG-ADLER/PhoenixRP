...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.GasStation) do
		if not DoesBlipExist(blip[k]) then
			...
			BeginTextCommandSetBlipName('pompebenzin')
			AddTextEntry('pompebenzin', 'Gas station')
...
RegisterNUICallback('pay', function(data, cb)
	...
	if not paid then
		if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(vehicle)) < 5 then
			TriggerServerEvent('renzu_fuel:payfuel',math.floor(new_perc),VehToNet(vehicle),math.floor(new_perc),Config.FuelDecor,pumpLocation)
			paid = true
		end
	end
end)
