$(document).ready(function(){
      ...
      $(document).keyup(function(e) {
        if (!counting) {
          if (e.key === "Escape") {
            ...
            completar;
            maxFuel;
            $.post('http://PX_fuel/escape', JSON.stringify({}));
          }
        }
      });
    } else {
  ...
  function increase() {
    if (counting) {
      if (fuel < maxFuel) {
        ...
        $.post('http://PX_fuel/removeanim', JSON.stringify({}));
        if (completar) {
          console.log(perc_new*dataGas.price)
          console.log(price)
          $.post('http://PX_fuel/pay', JSON.stringify({ new_perc: perc_new*dataGas.price }));
          perc_new;
          completar = !completar;
        } else if (guardar) {
          console.log(perc_new*dataGas.price)
          console.log(price)
          $.post('http://PX_fuel/pay', JSON.stringify({ new_perc: price }));
          price;
          guardar = !guardar;
        }
        counting = !counting;
  ...
  $( "#retirar" ).click(function() {
    if (!counting) {
      ...
      $.post('http://PX_fuel/checkpay', JSON.stringify({ new_perc: perc_new }));
      counting = !counting;
      completar = !completar;
      maxFuel = 99
    ...
    if (!counting) {
      ...
      if (price > dataGas.price) {  
        ...
        if (price <= ((100-Math.round(datafuel))*dataGas.price)) {
          $.post('http://PX_fuel/checkpay', JSON.stringify({ new_perc: price }));
          counting = !counting;
          guardar = !guardar;
          ...
          $.post('http://PX_fuel/notifytext', JSON.stringify({ text: texto }));
        }
      } else {
  ...
  $(".accept").click(function() {
    if (counting) {
      ...
      $('left').fadeIn(300);
      $('.act').fadeIn(300);
      $.post('http://PX_fuel/startanim', JSON.stringify({}));
      myStop();
      myStart(); 
    }
  })
