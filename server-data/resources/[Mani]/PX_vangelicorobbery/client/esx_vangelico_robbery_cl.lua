...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("EZ_Pixel:vangelico:ImLoaded")
end)

Code = nil
Got = false
RegisterNetEvent("EZ_Pixel:vangelico:sendCode")
AddEventHandler("EZ_Pixel:vangelico:sendCode", function(code)
    if not Got then
    	Code = code
		Got = true
	else
...
AddEventHandler('esx_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
	ESX.ShowNotification(_U('robbery_cancelled'), 'info')
	robbingName = ""
	incircle = false
end)
...
AddEventHandler('esx_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	ESX.ShowNotification(_U('robbery_complete'), 'success')
	store = ""
	incircle = false
end)
...
Citizen.CreateThread(function()
	...
	while true do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						...
						incircle = true
						if IsPedShooting(GetPlayerPed(-1)) and not inPaintBall then
		...
		if holdingup then
			ESX.ShowMissionText(_U('smash_case') .. ' :~r~ ' .. vetrineRotte .. '/' .. Config.MaxWindows)
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
					    ...
					    TriggerServerEvent('esx_vangelico_robbery:gioielli', Code)
						...
						if vetrineRotte == Config.MaxWindows then 
						    ...
						    ESX.ShowNotification(_U('lester'), 'info')
						    holdingup = false
						    StopSound(soundid)
						end
					end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(60000)
		for k,v in pairs(Stores)do
		TriggerServerEvent('Mani:checkvantime',k)
		end
	end
end)
Citizen.CreateThread(function()
