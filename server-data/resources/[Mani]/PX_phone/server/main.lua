...
RegisterServerEvent('PX_phone:saveTwitterToDatabase')
AddEventHandler('PX_phone:saveTwitterToDatabase', function(firstName, lastname, message, url, time, picture)
...
RegisterServerEvent('PX_phone:server:AddAdvert')
AddEventHandler('PX_phone:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('PX_phone:client:UpdateAdverts', -1, Adverts, "@" .. character.firstname .. "" .. character.lastname)
end)
...
RegisterServerEvent('PX_phone:server:updateForEveryone')
AddEventHandler('PX_phone:server:updateForEveryone', function(newTweet)
    local src = source
    TriggerClientEvent('PX_phone:updateForEveryone', -1, newTweet)
end)

RegisterServerEvent('PX_phone:server:updateidForEveryone')
AddEventHandler('PX_phone:server:updateidForEveryone', function()
    TriggerClientEvent('PX_phone:updateidForEveryone', -1)
end)


ESX.RegisterServerCallback('PX_phone:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local character = GetCharacter(src)
...
RegisterServerEvent('PX_phone:deleteTweet')
AddEventHandler('PX_phone:deleteTweet', function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.execute('DELETE FROM twitter_tweets WHERE owner = @owner AND id = @id', {['@owner'] = xPlayer.identifier, ['@id'] = id})
end)


ESX.RegisterServerCallback('PX_phone:server:GetCallState', function(source, cb, ContactData)
...
RegisterServerEvent('PX_phone:server:SetCallState')
AddEventHandler('PX_phone:server:SetCallState', function(bool)
    local src = source
...
RegisterServerEvent('PX_phone:server:RemoveMail')
AddEventHandler('PX_phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    SetTimeout(100, function()
        ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `identifier` = "'..Player.identifier..'" ORDER BY `date` ASC', function(mails)
            ...
            TriggerClientEvent('PX_phone:client:UpdateMails', src, mails)
        end)
    end)
end)
...
RegisterServerEvent('PX_phone:server:sendNewMail')
AddEventHandler('PX_phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('PX_phone:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('PX_phone:server:sendNewMailToOffline')
AddEventHandler('PX_phone:server:sendNewMailToOffline', function(steam, mailData)
...
RegisterServerEvent('PX_phone:server:sendNewEventMail')
AddEventHandler('PX_phone:server:sendNewEventMail', function(steam, mailData)
    if mailData.button == nil then
        ExecuteSql(false, "INSERT INTO `player_mails` (`identifier`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES ('"..identifier.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0')")
    else
        ExecuteSql(false, "INSERT INTO `player_mails` (`identifier`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES ('"..identifier.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."')")
...
RegisterServerEvent('PX_phone:server:ClearButtonData')
AddEventHandler('PX_phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('PX_phone:server:MentionedPlayer')
AddEventHandler('PX_phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(ESX.GetPlayers()) do
        ...
        if Player ~= nil then
            if (character.firstname == firstName and character.lastname == lastName) then
                ...
                TriggerClientEvent('PX_phone:client:GetMentioned', Player.source, TweetMessage, AppAlerts[Player.identifier]["twitter"])
...
RegisterServerEvent('PX_phone:server:CallContact')
AddEventHandler('PX_phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = ESX.GetPlayerFromId(src)
    ...
    if Target ~= nil then
        TriggerClientEvent('PX_phone:client:GetCalled', Target.source, character.phone, CallId, AnonymousCall)
    end
end)

-- ESX(V1_Final) Fix
ESX.RegisterServerCallback('PX_phone:server:GetBankData', function(source, cb)
    local src = source
...
-- ESX(V1_Final) Fix
ESX.RegisterServerCallback('PX_phone:server:CanPayInvoice', function(source, cb, amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('PX_phone:server:GetInvoices', function(source, cb)
...
RegisterServerEvent('PX_phone:server:UpdateHashtags')
AddEventHandler('PX_phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('PX_phone:client:UpdateHashtags', -1, Handle, messageData)
end)
...
ESX.RegisterServerCallback('PX_phone:server:GetContactPictures', function(source, cb, Chats)
...
ESX.RegisterServerCallback('PX_phone:server:GetContactPicture', function(source, cb, Chat)
...
ESX.RegisterServerCallback('PX_phone:server:GetPicture', function(source, cb, number)
...
RegisterServerEvent('PX_phone:server:SetPhoneAlerts')
AddEventHandler('PX_phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local Identifier = ESX.GetPlayerFromId(src).identifier
    MIPhone.SetPhoneAlerts(Identifier, app, alerts)
end)

RegisterServerEvent('PX_phone:server:UpdateTweets')
AddEventHandler('PX_phone:server:UpdateTweets', function(TweetData, type)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('PX_phone:client:UpdateTweets', -1, src, TwtData, type)
end)




RegisterServerEvent('PX_phone:server:TransferMoney')
AddEventHandler('PX_phone:server:TransferMoney', function(iban, amount)
    local src = source
        ...
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                if PhoneItem ~= nil then
                    TriggerClientEvent('PX_phone:client:TransferMoney', recieverSteam.source, amount, recieverSteam.bank)
...
RegisterServerEvent('PX_phone:server:EditContact')
AddEventHandler('PX_phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('PX_phone:server:RemoveContact')
AddEventHandler('PX_phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('PX_phone:server:AddNewContact')
AddEventHandler('PX_phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('PX_phone:server:UpdateMessages')
AddEventHandler('PX_phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
            ...
            if TargetData ~= nil then
                ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        TriggerClientEvent('PX_phone:client:UpdateMessages', TargetData.source, ChatMessages, ChatNumber, false)
                    else
                        -- Insert for target
                        ...
                        -- Send notification & Update messages for target
                        TriggerClientEvent('PX_phone:client:UpdateMessages', TargetData.source, ChatMessages, ChatNumber, true)
                    end
                end)
            else
                ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'", function(Chat)
...
RegisterServerEvent('PX_phone:server:AddRecentCall')
AddEventHandler('PX_phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = ESX.GetPlayerFromId(src)
    local character = GetCharacter(src)
    ...
    TriggerClientEvent('PX_phone:client:AddRecentCall', src, data, label, type)

    local Trgt = GetPlayerFromPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('PX_phone:client:AddRecentCall', Trgt.source, {
...
RegisterServerEvent('PX_phone:server:CancelCall')
AddEventHandler('PX_phone:server:CancelCall', function(ContactData)
    local Ply = GetPlayerFromPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('PX_phone:client:CancelCall', Ply.source)
    end
end)

RegisterServerEvent('PX_phone:server:AnswerCall')
AddEventHandler('PX_phone:server:AnswerCall', function(CallData)
    local Ply = GetPlayerFromPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('PX_phone:client:AnswerCall', Ply.source)
    end
end)

RegisterServerEvent('PX_phone:server:SaveMetaData')
AddEventHandler('PX_phone:server:SaveMetaData', function(column,data)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('PX_phone:server:FetchResult', function(source, cb, search)
    local src = source
...
ESX.RegisterServerCallback('PX_phone:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
    local character = GetCharacter(src)
...
ESX.RegisterServerCallback('PX_phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    local character = GetCharacter(src)
    if plate ~= nil then 
...
ESX.RegisterServerCallback('PX_phone:server:GetGarageVehicles', function(source, cb)
...
ESX.RegisterServerCallback('PX_phone:server:GetCharacterData', function(source, cb,id)
...
-- Inventory Fix
ESX.RegisterServerCallback('PX_phone:server:HasPhone', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('PX_phone:server:GiveContactDetails')
AddEventHandler('PX_phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local character = GetCharacter(src)
    ...
    TriggerClientEvent('PX_phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('PX_phone:server:AddTransaction')
AddEventHandler('PX_phone:server:AddTransaction', function(data)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('PX_phone:server:GetCurrentpolices', function(source, cb)
    local polices = {}
    for k, v in pairs(ESX.GetPlayers()) do
        local Player = ESX.GetPlayerFromId(v)
        local character = GetCharacter(v)
        ...
        if Player ~= nil then
            if (Player.job.name == "casino" or Player.job.name == "lawyer" or Player.job.name == "ambulance" or Player.job.name == "taxi" or Player.job.name == "mechanic" or Player.job.name == "realstate" or Player.job.name == "weazel") then
