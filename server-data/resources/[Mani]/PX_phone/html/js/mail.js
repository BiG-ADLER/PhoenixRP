...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://PX_phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://PX_phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
MI.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(MI.Phone.Data.PlayerData.charinfo.phone+"@gmail.com");
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">There is no incoming mail!</p>');
        }
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://PX_phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
...
MI.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">No advertising message sent!</span></div>';
        $(".advert-list").append(element);
    }
}