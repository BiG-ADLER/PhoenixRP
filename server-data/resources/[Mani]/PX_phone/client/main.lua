...
RegisterNetEvent('PX_phone:client:RaceNotify')
AddEventHandler('PX_phone:client:RaceNotify', function(message)
...
RegisterNetEvent('PX_phone:client:AddRecentCall')
AddEventHandler('PX_phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('PX_phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('PX_phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('PX_phone:server:SaveMetaData', 'background', background)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            ESX.TriggerServerCallback('PX_phone:server:GetPhoneData', function(pData)
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                    --print(PhoneData.Tweets[j].message)
                        TriggerServerEvent('PX_phone:server:MentionedPlayer', Firstname, Lastname, PhoneData.Tweets[j])
                end
            end
        end
    end
...
RegisterNUICallback('HasPhone', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:HasPhone', function(HasPhone)
         cb(HasPhone)
     end)
 end)
...
function OpenPhone()
    ESX.TriggerServerCallback('PX_phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ESX.TriggerServerCallback('PX_phone:server:GetCharacterData', function(chardata)
                PhoneData.PlayerData = ESX.GetPlayerData()
                ...
                -- Garage Fix
                ESX.TriggerServerCallback('PX_phone:server:GetGarageVehicles', function(vehicles)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('PX_phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('PX_phone:client:UpdateMails')
AddEventHandler('PX_phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('PX_phone:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('PX_phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    ESX.TriggerServerCallback('PX_phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('GetBankData', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:GetBankData', cb)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('PX_phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('PX_phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    ESX.TriggerServerCallback('PX_phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('PX_phone:client:UpdateMessages')
AddEventHandler('PX_phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    SendNUIMessage({
        action = "Notification",
        NotifyData = {
            title = Lang("WHATSAPP_TITLE"),
...
RegisterNetEvent("PX_phone:client:BankNotify")
AddEventHandler("PX_phone:client:BankNotify", function(text)
    SendNUIMessage({
        action = "Notification",
        NotifyData = {
...
RegisterNetEvent('PX_phone:client:NewMailNotify')
AddEventHandler('PX_phone:client:NewMailNotify', function(MailData)
    if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
    ...
    TriggerServerEvent('PX_phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('PX_phone:server:AddAdvert', data.message)
end)

RegisterNetEvent('PX_phone:client:UpdateAdverts')
AddEventHandler('PX_phone:client:UpdateAdverts', function(Adverts, LastAd)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('PX_phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    ESX.TriggerServerCallback('PX_phone:server:CanPayInvoice', function(CanPay)
        if CanPay then
...
function PayInvoice(cb,invoiceId)
    ...
    ESX.TriggerServerCallback('esx_billing:payBill', function()
        ESX.TriggerServerCallback('PX_phone:server:GetInvoices', function(Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    ESX.TriggerServerCallback('PX_phone:server:DeclineInvoice', function(CanPay, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('PX_phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('PX_phone:client:UpdateHashtags')
AddEventHandler('PX_phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('DeleteTweet', function(data, cb)
    TriggerServerEvent("PX_phone:deleteTweet", data.id)
    ...
    TriggerServerEvent('PX_phone:server:updateForEveryone', PhoneData.Tweets)
end)
...
RegisterNUICallback('UpdateProfilePicture', function(data)
    ...
    TriggerServerEvent('PX_phone:server:SaveMetaData', 'profilepicture', pf)
end)
local test

local patt = "[?!@#]"

RegisterNetEvent("PX_phone:updateForEveryone")
AddEventHandler("PX_phone:updateForEveryone", function(newTweet)
    PhoneData.Tweets = newTweet
end)

RegisterNetEvent("PX_phone:updateidForEveryone")
AddEventHandler("PX_phone:updateidForEveryone", function()
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    test = ""
    TriggerServerEvent("PX_phone:saveTwitterToDatabase", TweetMessage.firstName, TweetMessage.lastName, TweetMessage.message, TweetMessage.url, TweetMessage.time, TweetMessage.picture)
   TriggerServerEvent("PX_phone:server:updateidForEveryone")
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('PX_phone:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('PX_phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                else
    ...
    TriggerServerEvent('PX_phone:server:updateForEveryone', PhoneData.Tweets)
    cb(PhoneData.Tweets)
    TriggerServerEvent('PX_phone:server:UpdateTweets', TweetMessage)
    SendNUIMessage({
        action= "updateTest",
...
RegisterNetEvent('PX_phone:client:TransferMoney')
AddEventHandler('PX_phone:client:TransferMoney', function(amount, newmoney)
    if PhoneData.isOpen then
        SendNUIMessage({ action = "UpdateBank", NewBalance = newmoney })
    end
end)

RegisterNetEvent('PX_phone:client:UpdateTweets')
AddEventHandler('PX_phone:client:UpdateTweets', function(src, NewTweetData)
...
RegisterNetEvent('PX_phone:client:GetMentioned')
AddEventHandler('PX_phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('PX_phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('PX_phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, callback)
    ...
    ESX.TriggerServerCallback('PX_phone:server:GetBankData', function(bankdata)
        if tonumber(bankdata.bank) >= amount then
            local amaountata = tonumber(bankdata.bank) - amount
            TriggerServerEvent('PX_phone:server:TransferMoney', data.iban, amount)
            local cbdata = {
                CanTransfer = true,
...
RegisterNUICallback('GetWhatsappChats', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:GetCallState', function(CanCall, IsOnline)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('PX_phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('PX_phone:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('PX_phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('PX_phone:server:SetCallState', false)
...
RegisterNetEvent('PX_phone:client:CancelCall')
AddEventHandler('PX_phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('PX_phone:client:GetCalled')
AddEventHandler('PX_phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('PX_phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('PX_phone:server:AnswerCall', PhoneData.CallData)
    if Config.Tokovoip then
...
RegisterNetEvent('PX_phone:client:AnswerCall')
AddEventHandler('PX_phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    ESX.TriggerServerCallback('PX_phone:server:ScanPlate', function(result)
...
RegisterNetEvent('PX_phone:client:addPoliceAlert')
AddEventHandler('PX_phone:client:addPoliceAlert', function(alertData)
...
RegisterNetEvent('PX_phone:client:GiveContactDetails')
AddEventHandler('PX_phone:client:GiveContactDetails', function()
    local ped = PlayerPedId()

    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('PX_phone:server:GiveContactDetails', PlayerId)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('PX_phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('PX_phone:client:AddNewSuggestion')
AddEventHandler('PX_phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('PX_phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)
...
RegisterNetEvent('PX_phone:client:RemoveBankMoney')
AddEventHandler('PX_phone:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('PX_phone:client:AddTransaction')
AddEventHandler('PX_phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('PX_phone:server:AddTransaction', Data)
end)
...
RegisterNetEvent('PX_phone:client:UpdateLapraces')
AddEventHandler('PX_phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetPlayerHouses', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)
...
RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('GetCurrentpolices', function(data, cb)
    ESX.TriggerServerCallback('PX_phone:server:GetCurrentpolices', function(polices)
        cb(polices)
    end)
end)
