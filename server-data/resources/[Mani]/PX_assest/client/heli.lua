...
local toggle_spotlight = 183 -- control id to toggle the various spotlight states Default: INPUT_PhoneCameraGrid (G-183) - Changed to 173, ArrowDown
...
local lightup_key = 174 -- control id to increase spotlight brightness. Default: INPUT_MP_TEXT_CHAT_TEAM (Y) - Changed to 174 (left arrow)

local lightdown_key = 175 -- control id to decrease spotlight brightness. Default: INPUT_CELLPHONE_DOWN  (ARROW-DOWN) - Changed to 175 (right arrow)
...
local polmav_hash = GetHashKey("polmav") --Police Maverick

local chopper_hash = GetHashKey("buzzard") -- KUFFS Custom PolMav

local mh60t_hash = GetHashKey("buzzard2") -- MH60T Jayhawk

local mh65c_hash = GetHashKey("valkyrie") -- MH65C Dolphin

local swift_hash = GetHashKey("valkyrie2") -- EMS Swift

local medevac_hash = GetHashKey("maverick") -- MedEvac

local newsmav_hash = GetHashKey("supervolito2") -- News Maverick
...
function IsPlayerInPolmav()
    ...
    local IsChopper = IsVehicleModel(vehicle, chopper_hash)

    local IsMH60T = IsVehicleModel(vehicle, mh60t_hash)

	local IsMH65C = IsVehicleModel(vehicle, mh65c_hash)

    local IsSwift = IsVehicleModel(vehicle, swift_hash)

    local IsMedEvac = IsVehicleModel(vehicle, medevac_hash)

	local IsNewsMav = IsVehicleModel(vehicle, newsmav_hash)

    local isCorrect = false

    if( IsPolmav or IsChopper or IsMH60T or IsMH65C or IsSwift or IsMedEvac or IsNewsMav) then
