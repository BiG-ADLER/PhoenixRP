...
function syncRadioData(radioTable, list)
...
function setTalkingOnRadio(plySource, enabled, pname)
	toggleVoice(plySource, enabled, 'radio')
	radioData[plySource] = enabled
	playMicClicks(enabled)
	exports["rp-radio"]:updateIndicators("radio", radioData)
end
...
function addPlayerToRadio(plySource, name)
	if GetPlayerServerId(PlayerId()) == plySource then return end
...
function setRadioChannel(channel)
	...
	TriggerEvent("PX_hud:updateRadioState", {enabled = radioEnabled, channel = radioChannel})
end
...
RegisterCommand('+radooii', function()
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
...
RegisterKeyMapping('+radooii', 'Herf Zaadan Dar Radio', 'keyboard', "capital")
