...
Citizen.CreateThread(function()
	...
	for k, v in pairs(Config.Shops) do
		...
		BeginTextCommandSetBlipName("Tattoo Shop")
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Shoma Tattoo ~y~" .. GetLabelText(label) .. "~s~ Ra ~r~Remove~s~ Kardid!", 'info')
end
						...
						if GetEntityModel(PlayerPedId()) == 'mp_m_freemode_01' then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									...
									local price = math.ceil(tattoo.Price) == 0 and 100 or math.ceil(tattoo.Price / 10)
